...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Winsorize price at 99.9th percentile ─────────────────────────────────
    ...
    # We cap before scaling but keep the raw price for reference.
    p999 = df['price_mwh'].quantile(0.999)
    print(f"99.9th percentile price : ${p999:.2f}/MWh")
    print(f"Prices above this cap   : {(df['price_mwh'] > p999).sum()} hours")
...
if __name__ == '__main__':
    ...
    # Save datetime + all feature cols
    cols_to_save = ['datetime'] + FEATURE_COLS + \
